...
public class ExternalApproveService {
    ...
    private int approveCar(CollateralObject object) {
        if (object.getYear() < MIN_CAR_YEAR) { //Если дата выпуска авто меньше мин даты то -10
            return -10;
        }
        if (object.getDate().isBefore(MIN_ASSESS_DATE)) {//Если дата до минимальной даты то -11
            return -11;
        }
        if (object.getValue().compareTo(MIN_CAR_VALUE) < 0) {//Если значение оценки меньше минимального то -1, если больше то 1
            return -12;
        }
